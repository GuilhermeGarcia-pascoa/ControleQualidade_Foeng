...
// ─── POST /api/registos — aceita multipart/form-data ──────
app.post('/api/registos', upload.any(), async (req, res) => {
  try {
    const { no_id, utilizador_id, dados_json } = req.body;

    // Reconstruir o objeto de dados com os caminhos das fotos
    const dados = JSON.parse(dados_json);

    // Para cada ficheiro recebido, substitui o valor no objeto dados
    // O nome do campo no multipart é o nome do campo dinâmico
