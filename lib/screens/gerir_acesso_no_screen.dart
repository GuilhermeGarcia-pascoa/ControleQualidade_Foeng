...
class _GerirAcessoNoScreenState extends State<GerirAcessoNoScreen> with SingleTickerProviderStateMixin {
  ...
  void _loadMembros() async {
    ...
    // ✅ CORRIGIDO: Agora usa getMembrosNo correspondente ao database_helper.dart
  ...
  void _adicionarAcesso() async {
    ...
    try {
      ...
      if (jaExiste) {
        _showSnackBar('Este utilizador já tem acesso a esta pasta!', Colors.orange, Icons.warning_amber_rounded);
        return;
      }

      // ✅ CORRIGIDO: Agora usa darAcessoNo correspondente ao database_helper.dart
      final sucesso = await DatabaseHelper.instance.darAcessoNo(
        utilizador['id'],
        widget.no.id!,
      );
      ...
      if (sucesso) {
        ...
        _loadMembros();
        _showSnackBar('${utilizador['nome']} agora tem acesso!', Colors.green, Icons.check_circle_outline_rounded);
      } else {
        _showSnackBar('Erro: Não foi possível dar acesso.', Colors.red, Icons.error_outline_rounded);
      }
  ...
  void _removerAcesso(Map<String, dynamic> membro) async {
        ...
        title: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.security_update_warning_rounded, color: Colors.red, size: 22),
            ),
            const SizedBox(width: 12),
            const Expanded(child: Text('Remover Acesso', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
          ],
        ),
        content: RichText(
          text: TextSpan(
            ...
            children: [
              ...
              const TextSpan(text: ' a esta pasta?'),
            ],
          ),
        ),
        actions: [
    ...
    if (confirm == true) {
      // ✅ Usa o removerAcessoNo - Este já estava certinho com o DB Helper
      await DatabaseHelper.instance.removerAcessoNo(membro['id'], widget.no.id!);
      _loadMembros();
      _showSnackBar('Acesso removido com sucesso.', Colors.grey.shade700, Icons.info_outline_rounded);
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            const Text('Gerir Acessos', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            Text(widget.no.nome, style: TextStyle(fontSize: 12, color: theme.colorScheme.onSurface.withOpacity(0.6))),
          ],
        ),
        actions: [
          if (!_loading && membros.isNotEmpty)
            Center(
              child: Container(
                ...
                child: Row(
                  children: [
                    Icon(Icons.lock_open_rounded, color: theme.colorScheme.primary, size: 14),
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: RadialGradient(
                  colors: [theme.colorScheme.secondary.withOpacity(isDark ? 0.15 : 0.08), Colors.transparent],
                  stops: const [0.2, 1.0],
                ),
              ),
            ),
            ...
            child: Column(
              children: [
                      ...
                      Container(
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              const SizedBox(width: 12),
                              Icon(Icons.person_add_alt_1_rounded, color: theme.colorScheme.onSurface.withOpacity(0.5), size: 20),
                              const SizedBox(width: 12),
                              Expanded(
                                child: TextField(
                                  controller: emailC,
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      Container(width: 4, height: 16, decoration: BoxDecoration(color: theme.colorScheme.secondary, borderRadius: BorderRadius.circular(2))),
                      const SizedBox(width: 10),
                      Text('UTILIZADORES COM ACESSO', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.5), fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 0.8)),
                    ],
                  ),
                ),
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Container(
                                      ...
                                      child: Icon(Icons.no_accounts_outlined, color: theme.disabledColor, size: 36),
                                    ),
                                    const SizedBox(height: 20),
                                    Text('Nenhum acesso extra.', style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.w600)),
                                    const SizedBox(height: 8),
                                    Text('Os administradores já têm acesso.\nAdiciona permissões específicas acima.', textAlign: TextAlign.center, style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.6), fontSize: 14, height: 1.4)),
                                  ],
                                ),
                              ),
                            )
                          : FadeTransition(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final cor = isAdmin ? theme.colorScheme.primary : theme.colorScheme.secondary;
